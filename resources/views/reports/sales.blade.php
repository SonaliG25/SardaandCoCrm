...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="bg-white rounded-lg shadow-sm p-4 mb-6">
        <form method="GET" action="{{ route('reports.sales') }}" class="flex items-end space-x-4">
            <div class="flex-1">
    ...
    <div class="rounded-lg shadow-lg p-6" style="background: linear-gradient(to bottom right, #f2601f, #e34715);">
        <div class="flex items-center justify-between">
            <div style="color: white;">
                ...
                <p class="text-3xl font-bold">₹{{ number_format($stats['average_order_value']) }}</p>
            </div>
    ...
    <div class="rounded-lg shadow-lg p-6" style="background: linear-gradient(to bottom right, #f2601f, #e34715);">
        <div class="flex items-center justify-between">
            <div style="color: white;">
                ...
                <p class="text-3xl font-bold">{{ number_format($stats['delivered_orders']) }}</p>
            </div>
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center space-x-3 flex-1">
                    <div class="w-10 h-10 rounded-full flex items-center justify-center
                        @if($method->payment_gateway == 'razorpay') bg-purple-100
                        @elseif($method->payment_gateway == 'cod') bg-yellow-100
                        @else bg-gray-100
                        @endif">
                        <i class="fas 
                            @if($method->payment_gateway == 'razorpay') fa-credit-card text-purple-600
                            @elseif($method->payment_gateway == 'cod') fa-money-bill text-yellow-600
                            @else fa-wallet text-gray-600
                            @endif"></i>
                    </div>
                    <div class="flex-1">
                        <p class="font-medium text-gray-900">{{ ucfirst($method->payment_gateway ?? 'Other') }}</p>
                        <div class="w-full bg-gray-200 rounded-full h-2 mt-1">
                            <div class="bg-sarda-600 h-2 rounded-full" 
                                 style="width: {{ ($method->revenue / $stats['total_revenue']) * 100 }}%"></div>
                        </div>
                    </div>
                </div>
                <div class="text-right ml-4">
                    <p class="font-bold text-gray-900">₹{{ number_format($method->revenue) }}</p>
